"use client"

import { signIn } from 'next-auth/react'
import NautilusLogo from '@/components/NautilusLogo'

export default function SignInPage() {
  return (
    <main className="min-h-screen bg-[#030303] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        {/* Brand */}
        <div className="flex items-center gap-3 mb-10 justify-center">
          <NautilusLogo size={28} className="text-sky-400" />
          <div>
            <span className="text-[10px] font-bold tracking-[0.35em] text-sky-400 uppercase block">
              NAUTILUS
            </span>
            <span className="text-[8px] text-slate-600 uppercase tracking-[0.2em]">
              Terminal
            </span>
          </div>
        </div>

        {/* Card */}
        <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-8 shadow-[0_0_60px_rgba(56,189,248,0.06)]">
          <h1 className="text-white font-bold text-lg mb-1">Sign in</h1>
          <p className="text-slate-500 text-xs mb-8 leading-relaxed">
            Access the Nautilus supply chain intelligence terminal.
          </p>

          <button
            onClick={() => signIn('google', { callbackUrl: '/terminal' })}
            className="w-full flex items-center justify-center gap-3 bg-white text-[#0a0a0a] font-semibold text-sm h-12 rounded-xl hover:bg-slate-100 active:bg-slate-200 transition-colors"
          >
            <GoogleIcon />
            Continue with Google
          </button>
        </div>

        <p className="text-center text-[10px] text-slate-700 mt-6">
          By signing in you agree to our{' '}
          <a href="/legal/terms" className="underline hover:text-slate-500">Terms</a>
          {' '}and{' '}
          <a href="/legal/disclaimer" className="underline hover:text-slate-500">Disclaimer</a>.
        </p>
      </div>
    </main>
  )
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
      <path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 01-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z" />
      <path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 009 18z" />
      <path fill="#FBBC05" d="M3.964 10.71A5.41 5.41 0 013.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 000 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" />
      <path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 00.957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z" />
    </svg>
  )
}
